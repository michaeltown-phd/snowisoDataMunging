...
This is the same as snowCoreDepthAgeCalc.py but it extends the bottom values down by 
the mean accumulation values for the previous six months
...
# function for accumulation determination at end of script
def safeInverse(x):
    if x == 0:
        return np.nan
    else:
...
# input which depth model to use

depthModel = input('Which depth model to use in creating data set - meanAccBot (m), constrainedBot (c): ')
depthModel = depthModel.lower()


# load the sonic ranger accumulation data
fileLocSonic = '/home/michaeltown/work/projects/snowiso/data/EastGRIP/meteo/'
dfSonicAcc = pd.read_pickle(fileLocSonic+'accumulationDataSonicEastGRIP2014-2019.pkl')


# list the data
fileLocMeanAccBot = '/home/michaeltown/work/projects/snowiso/data/EastGRIP/isotopes/depthAge_meanAccBot/'
fileLocConstrBot = '/home/michaeltown/work/projects/snowiso/data/EastGRIP/isotopes/depthAge_constrainBot/'
fileLocData = '/home/michaeltown/work/projects/snowiso/data/EastGRIP/isotopes/'

if depthModel == 'm':
    fileLoc = fileLocMeanAccBot
    modelStr = 't2'
elif depthModel == 'c':
    fileLoc = fileLocConstrBot
    modelStr = 't3'
else:
    print("I don't recognize that depth model.")
    exit()
...
df_st = pd.read_pickle('../surfaceTransectDataEastGRIP2016-2019.pkl')
...
for y in years:
    ...
    for c in coreIDs:
        ...
        plt.legend()
        fm.saveas(fig,figureLoc+'ageDepthSlopes_core_'+modelStr+'_'+str(c)+'_'+str(y))    
...
fm.saveas(fig,figureLoc+'ageDepthSlopes_core_1-5'+str(y))
    ...
    for c in coreIDs:
        ...
        for d in dates:
            ...
            if len(dfTemp) > 32:
                ...
                # using the mean accumulation rate for the previous three months
                # we construct a time series assuming that each depth interval is 2 cm
                if str(dfTemp.iloc[max(dfDay.index),3]) == 'NaT':
                    ...
                    # converts accumulation from m/month to cm/day, 
                    # could be more precise here by determining the number of 
                    #days in each month
                    
                    
                    acc_mean = np.mean(dfSonicAcc[dfSonicAcc.index <= beg].Sonic_mean[-(periodNum*2):]*100/30)
                    
                    depthInterval_dt = dt.timedelta(days = periodNum*2/acc_mean)     # assumes accumulation is in cm/day
                    end = beg-depthInterval_dt
                    timeRangeTemp = pd.date_range(beg,end,periods = periodNum+1)
                    dfDay.iloc[begInd:endInd+1,dfDay.columns.get_loc('snowDatedt')]=timeRangeTemp
                ...
                fm.saveas(fig,figureLoc+'d18OvsDepth_Age_'+modelStr+'_c'+str(c)+'_'+str(d)[0:10].replace('-',''))
                ...
                # assign the dates to the original df_iso data frame
                df_iso_m.loc[dfTemp.index,'snowDatedt'] = dfTemp.snowDatedt            
                fm.saveas(fig,figureLoc+'dxsVsDepth_Age_'+modelStr+'_c'+str(c)+'_'+str(d)[0:10].replace('-',''))
...
for c in coreIDs:
    ...
    fm.saveas(fig,figureLoc+'d18OvsAge_'+modelStr+'_c'+str(c)+'_'+str(np.min(years))+'-'+str(np.max(years)))
...
for c in coreIDs:
    ...
    fm.saveas(fig,figureLoc+'dxsvsAge_'+modelStr+'_c'+str(c)+'_'+str(np.min(years))+'-'+str(np.max(years)))
...
for c in coreIDs:
    for y in years:
        ...
        fm.saveas(fig,figureLoc+'d18OvsAge_'+modelStr+'_c'+str(c)+'_'+str(y))
...
for c in coreIDs:
    for y in years:
        ...
        fm.saveas(fig,figureLoc+'dxsVsAge_'+modelStr+'_c'+str(c)+'_'+str(y))
...
df_iso = df_iso_m;
df_iso.to_pickle(fileLocData+'eastGRIP_SCisoData_2016-2019_acc_peaks_'+modelStr+'.pkl')
...
fig = plt.figure()
plt.plot(df_acc.snowDatedt,df_acc.da_slope.apply(safeInverse),'.',alpha = 0.5,label = 'snow cores')
...
plt.legend()
fm.saveas(fig,figureLoc+'accumulationRatesSonicSnowcoresEastGRIP2015-2019_'+modelStr)

# find the mean accumulation rate for the same time periods of both time series
beg = pd.to_datetime('20150101',format = '%Y%m%d')
end = pd.to_datetime('20190101',format = '%Y%m%d')
print('accumulation rate for sonic data in mm/day, +/- 1 std')
print(np.mean(dfSonicAcc[(dfSonicAcc.index >= beg)&(dfSonicAcc.index < end)].Sonic_mean*100/30))
print(np.std(dfSonicAcc[(dfSonicAcc.index >= beg)&(dfSonicAcc.index < end)].Sonic_mean*100/30))
print('accumulation rate for iso data in mm/day, +/- 1 std')
print(np.mean(df_acc[(df_acc.snowDatedt >= beg)&(df_acc.snowDatedt < end)].da_slope.apply(safeInverse)))
print(np.std(df_acc[(df_acc.snowDatedt >= beg)&(df_acc.snowDatedt < end)].da_slope.apply(safeInverse)))
