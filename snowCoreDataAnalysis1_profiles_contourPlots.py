...
alphas = np.arange(0.2,0.8,0.1) # coloring of the plots
...
#    df_dxs_p.rename(columns = dictCols, inplace = True)
    ...
    # plotting the residual d18O data with Promice data
    fig1, ax2  = plt.subplots(2,1);
    ...
    # plotting the dxs residual data with Promice data
    fig1, ax2  = plt.subplots(2,1);
    ax2[1].set(position = [0.1, 0.05, 0.65, 0.4])
...
# d18O mean contour plot
y = 2019;
df_d18O_p_ave = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).d18O.mean()
df_d18O_p_ave = df_d18O_p_ave.reset_index(); 
df_d18O_p_ave= df_d18O_p_ave.pivot(*df_d18O_p_ave.columns)                  # what does this * do here? something magical, ok pass a number of things to pivot

df_d18O_p_std = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).d18O.std()
df_d18O_p_std = df_d18O_p_std.reset_index(); 
df_d18O_p_std = df_d18O_p_std.pivot(*df_d18O_p_std.columns)

df_d18O_p_num = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).d18O.count()
df_d18O_p_num = df_d18O_p_num.reset_index(); 
df_d18O_p_num = df_d18O_p_num.pivot(*df_d18O_p_num.columns)

fig1, ax2  = plt.subplots(2,1);
cntr = ax2[1].contourf(df_d18O_p_ave.columns, -df_d18O_p_ave.index, df_d18O_p_ave.values, cmap = 'Greys',vmin = -50, vmax = -20)
ax2[1].set(position = [0.1, 0.05, 0.65, 0.4])
ax2[0].plot(df_promice[(df_promice.index>d1)&(df_promice.index<d2)].index,df_promice[(df_promice.index>d1)&(df_promice.index<d2)].AirTemperatureC)
ax2[0].set(position = [0.125, 0.55, 0.62, 0.4])
ax2[0].set_ylabel('T (oC)')
...
for date in dates:
    plt.text(date,-20,' |')
    plt.text(date,-18,'^')
plt.text(dates[1],-40,'mean d18O')

fig1.savefig(figureLoc + 'snowCoreEastGRIP_d18O_T_mean_contour.jpg')

fig1, ax2  = plt.subplots(2,1);
cntr = ax2[1].contourf(df_d18O_p_ave.columns, -df_d18O_p_std.index, df_d18O_p_std.values, cmap = 'Greys',vmin = 0, vmax = 10)
ax2[1].set(position = [0.1, 0.05, 0.65, 0.4])
ax2[0].plot(df_promice[(df_promice.index>d1)&(df_promice.index<d2)].index,df_promice[(df_promice.index>d1)&(df_promice.index<d2)].AirTemperatureC)
ax2[0].set(position = [0.125, 0.55, 0.62, 0.4])
ax2[0].set_ylabel('T (oC)')
...
for date in dates:
    plt.text(date,-20,' |')
    plt.text(date,-18,'^')
plt.text(dates[1],-40,'std d18O')
fig1.savefig(figureLoc + 'snowCoreEastGRIP_d18O_T_std_contour.jpg')


df_dxs_p_ave = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).dexcess.mean()
df_dxs_p_ave = df_dxs_p_ave.reset_index(); 
df_dxs_p_ave= df_dxs_p_ave.pivot(*df_dxs_p_ave.columns)                  # what does this * do here? something magical, ok pass a number of things to pivot

df_dxs_p_std = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).dexcess.std()
df_dxs_p_std = df_dxs_p_std.reset_index(); 
df_dxs_p_std = df_dxs_p_std.pivot(*df_dxs_p_std.columns)

df_dxs_p_num = df_test[(df_test.year == y)].groupby(['depthAcc_reg','date']).dexcess.count()
df_dxs_p_num = df_dxs_p_num.reset_index(); 
df_dxs_p_num = df_dxs_p_num.pivot(*df_dxs_p_num.columns)

# dxs mean plot
fig1, ax2  = plt.subplots(2,1);
cntr = ax2[1].contourf(df_dxs_p_ave.columns, -df_dxs_p_ave.index, df_dxs_p_ave.values, cmap = 'bwr',vmin = -10, vmax = 25)
ax2[1].set(position = [0.1, 0.05, 0.65, 0.4])
ax2[0].plot(df_promice[(df_promice.index>d1)&(df_promice.index<d2)].index,df_promice[(df_promice.index>d1)&(df_promice.index<d2)].AirTemperatureC)
ax2[0].set(position = [0.125, 0.55, 0.62, 0.4])
ax2[0].set_ylabel('T (oC)')
...
plt.text(dates[1],-40,'mean dxs')
fig1.savefig(figureLoc + 'snowCoreEastGRIP_dxs_T_mean_contour.jpg')

fig1, ax2  = plt.subplots(2,1);
cntr = ax2[1].contourf(df_dxs_p_ave.columns, -df_dxs_p_std.index, df_dxs_p_std.values, cmap = 'Greys',vmin = 0, vmax = 10)
ax2[1].set(position = [0.1, 0.05, 0.65, 0.4])
ax2[0].plot(df_promice[(df_promice.index>d1)&(df_promice.index<d2)].index,df_promice[(df_promice.index>d1)&(df_promice.index<d2)].AirTemperatureC)
ax2[0].set(position = [0.125, 0.55, 0.62, 0.4])
ax2[0].set_ylabel('T (oC)')
...
for date in dates:
    plt.text(date,-20,' |')
    plt.text(date,-18,'^')
plt.text(dates[1],-40,'std dxs')
fig1.savefig(figureLoc + 'snowCoreEastGRIP_dxs_T_std_contour.jpg')
